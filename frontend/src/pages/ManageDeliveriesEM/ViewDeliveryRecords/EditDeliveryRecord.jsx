...
const EditDeliveryRecord = () => {
  ...
  useEffect(() => {
    ...
    const fetchDeliveryRecord = async () => {
      try {
        ...
        // Format date and set delivery data
        ...
        // Find and set selected supplier if exists
        const supplier = suppliers.find(s => s.supplierId === data.supplierId);
        if (supplier) setSelectedSupplier(supplier);

        // Find and set selected driver if exists (and not self transport)
        if (data.transport && data.transport !== "selfTransport") {
          const driver = driverOptions.find(d => d.driverId === data.transport);
          if (driver) setSelectedDriver(driver);
        }
      } catch (error) {
        showAlert("Error fetching delivery data.", "error");
        console.error(error);
      }
    };
    ...
    fetchDrivers();
    fetchSuppliers();
    if (deliveryId) fetchDeliveryRecord();
  }, [deliveryId]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (parseFloat(deliveryData.totalWeight) < totalDeductions + parseFloat(deliveryData.randalu || 0)) {
